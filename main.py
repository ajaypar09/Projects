...
def _build_parser() -> argparse.ArgumentParser:
    ...
    lookup_parser.add_argument(
        "serial_number_pos",
        nargs="?",
        help="Serial number provided positionally instead of --serial-number",
    )
    lookup_parser.add_argument(
        "name_parts",
        nargs="*",
        help="Card name provided positionally instead of --name",
    )
    lookup_parser.add_argument(
        "--serial-number",
        dest="serial_number_opt",
        help="Exact serial number of the card",
    )
    lookup_parser.add_argument("--name", dest="name_opt", help="Exact card name")
    lookup_parser.add_argument(
        "--sales-limit",
        type=int,
        default=5,
...
def app(argv: list[str] | None = None) -> None:
    ...
    if args.command == "lookup":
        db.initialize()
        serial_pos = getattr(args, "serial_number_pos", None)
        name_parts = getattr(args, "name_parts", [])
        ...
        serial_number = _clean(getattr(args, "serial_number_opt", None))
        name = _clean(getattr(args, "name_opt", None))

        positional_name = _clean(" ".join(name_parts)) if name_parts else None

        if serial_number is None:
            serial_number = _clean(serial_pos)
        if name is None:
            name = positional_name

        if getattr(args, "serial_number_opt", None) is None and getattr(args, "name_opt", None) is None:
            serial_candidate = _clean(serial_pos)
            name_candidate = positional_name

            if serial_candidate and name_candidate:
                serial_has_digit = any(ch.isdigit() for ch in serial_candidate)
                name_has_digit = any(ch.isdigit() for ch in name_candidate)
                if not serial_has_digit and name_has_digit:
                    serial_number = name_candidate
                    name = serial_candidate
                else:
                    serial_number = serial_candidate
                    name = name_candidate
            elif serial_candidate and not name_candidate:
                if any(ch.isdigit() for ch in serial_candidate):
                    serial_number = serial_candidate
                    name = None
                else:
                    serial_number = None
                    name = serial_candidate
            elif name_candidate and not serial_candidate:
                serial_number = None
                name = name_candidate
